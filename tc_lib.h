...
namespace tc_lib
{
  ...
  { \
    uint32_t status=TC_GetStatus( \
      tc_lib::arduino_due::tc_info<\
        tc_lib::arduino_due::timer_ids::TIMER_TC##id \
      >::tc_p(), \
      tc_lib::arduino_due::tc_info<\
        tc_lib::arduino_due::timer_ids::TIMER_TC##id \
      >::channel \
    ...
    \
    tc_lib::arduino_due::capture< \
      tc_lib::arduino_due::timer_ids::TIMER_TC##id \
    >::tc_interrupt(status); \
  } \
  \
  typedef tc_lib::arduino_due::capture< \
    tc_lib::arduino_due::timer_ids::TIMER_TC##id \
  > capture_tc##id##_t; \
  \
  ...
  { \
    ...
    \
    tc_lib::arduino_due::action< \
      tc_lib::arduino_due::timer_ids::TIMER_TC##id \
    >::tc_interrupt(status); \
  } \
  \
  typedef tc_lib::arduino_due::action< \
    tc_lib::arduino_due::timer_ids::TIMER_TC##id \
  > action_tc##id##_t; \
  \
  ...
  namespace tc_lib 
  {
    ...
    namespace arduino_due 
    {
      ...
      struct period_callback_wrapper {
        period_callback_wrapper(period_callback_t callback = nullptr) {}
      };
        
      template<>
      struct period_callback_wrapper<true> 
      { 
        period_callback_wrapper(period_callback_t callback = nullptr): cllbck{callback} {}
      ...
      class capture 
      {
        public:
          ...
          bool config(
            ...
            period_callback_t period_callback = nullptr
          ...
          // the last valid measured duty and period. By default, the 
          ...
          // the status of the capture object. Returns in arguments 
          // the last valid measured duty and period, and the count of 
        ...
        private:
          ...
          struct _capture_ctx_: 
            period_callback_wrapper<WITH_CALLBACK> // using Empty Base Class Optimization
          {
            ...
            bool config(
              ...
              period_callback_t period_cllbck = nullptr
            )
            {
              return config_(
                the_capture_window,
  ...
  { \
    uint32_t status=static_cast<uint32_t>( \
      tc_lib::arduino_mkr::tc_info< \
        tc_lib::arduino_mkr::timer_ids::TIMER_TC##id \
      >::tc_p()->INTFLAG.reg \
    ); \
    \
    tc_lib::arduino_mkr::action< \
      tc_lib::arduino_mkr::timer_ids::TIMER_TC##id \
    >::tc_interrupt(status); \
  } \
  \
  typedef tc_lib::arduino_mkr::action< \
    tc_lib::arduino_mkr::timer_ids::TIMER_TC##id \
  > action_tc##id##_t; \
  \
  action_tc##id##_t action_tc##id;
    ...
    namespace arduino_mkr 
    {
      ...
      class action
      {
        ...
        public:
          ...
          constexpr uint32_t max_period() // hundreths of usecs. 
          { return static_cast<uint32_t>(_ctx_.max_period(VARIANT_MCK,16)*1.0e8); }
  
          // NOTE: get_period() returns 0 if the action is stopped
          uint32_t get_period() // hundreths of usecs. (1e-8 secs.)
          { return _ctx_.period; }

          uint32_t get_prescaler() { return _ctx_.prescaler; }

          uint32_t get_ticks()
          { return _ctx_.timer_ticks; }
  
          uint32_t ticks(uint32_t period) // hundreths of usecs. (1e-8 secs.)
          { 
            double period_s=static_cast<double>(period)*1.0e-8; 
            uint32_t prescaler;
            if(
              not timer::find_prescaler(
                period_s, 
                VARIANT_MCK, // 48 MHz
                16, // 16-bit counter mod
                prescaler
              )
            ) return 0;
            ...
            return timer::ticks(
              period_s,
              timer::tick_time(
                timer::prescaler_exponents[prescaler],
                VARIANT_MCK 
              )
            );
          }
      ...
      private:
        ...
        struct _action_ctx_
        {
          ...
          void init() 
          { period=0; prescaler=0; timer_ticks=0; callback=[](void* dummy){}; user_ctx=nullptr; }
  
          void tc_interrupt(uint32_t the_status)
          {
            // clearing interrrupt flags
            timer::info::tc_p()->INTFLAG.reg=TC_INTFLAG_MASK;
            
            // CC0 match interrupt?
            if(the_status & TC_INTFLAG_MC0) callback(user_ctx);
            
            // OVF interrupt?
            //if(the_status & TC_INTFLAG_OVF) callback(user_ctx);
          }
          ...
          void stop()
          {
            ...
            timer::disable_tc_interrupts();
            timer::stop_interrupts();

            // disabling tc
            timer::info::tc_p()->CTRLA.reg&=~TC_CTRLA_ENABLE;
            timer::wait_for_tc_synchronization();
  
            init();
          }
  
          static constexpr double max_period(uint32_t freq, uint32_t tc_bits) 
          { return timer::max_period(timer::max_prescaler_exponent,freq,tc_bits); }
      ...
      )
      {
        ...
        double period_s=static_cast<double>(the_period)*1.0e-8; 
        ...
        timer::info::tc_p()->CTRLA.reg=
          TC_CTRLA_PRESCSYNC_PRESC | // counter wrapping around on next prescaler clock
          TC_CTRLA_PRESCALER(prescaler) |
          TC_CTRLA_MODE_COUNT16 | // 16-bit counter
          TC_CTRLA_WAVEGEN_MFRQ; // MFRQ wave mode
        timer::wait_for_tc_synchronization();

        // setting the period
        timer::info::tc_p()->CC[0].reg=static_cast<uint16_t>(timer_ticks);
        timer::wait_for_tc_synchronization();
        
        timer::enable_tc_interrupts();
        timer::start_interrupts();
